...
if quantity_first_subjest >= 0 and quantity_first_subjest % 1 == 0 and quantity_second_subjest >= 0 and quantity_second_subjest % 1 == 0 and available_time >= full_time_hour and time_one_subjest >= 0:
    ...
    print(f"Время для первого предмета: {first_subjest_time} мин ", "/", f"{first_subjest_time_hour:.2f} ч")
    print(f"Время для второго предмета: {second_subjest_time} мин ", "/", f"{second_subjest_time_hour:.2f} ч")
    print(f"Время общей нагрузки: {full_time} мин ", "/", f"{full_time_hour:.2f} ч")
    print(f"Остаток свободного времени: {remaining_available_time:.2f} ч")
    print(f"Нагрузка за 4 одинаковые недели: {full_time_hour_week:.2f} ч")
